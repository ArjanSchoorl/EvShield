...
{
  if (i2c_address_a != Bank_A)
    bank_a.setAddress(i2c_address_a);
  if (i2c_address_b != Bank_B)
    bank_b.setAddress(i2c_address_b);
}
...
void EVShield::initProtocols(Protocols protocol)
{
  ...
  else
  {
    ...
    if (initCounter == 5)
    {
      ...
      pinMode(13, OUTPUT);
      while (true)
      { // stop here with red blinking light.
        ledSetRGB(100, 0, 0);
...
bool EVShield::getButtonState(uint8_t btn)
{
  ...
  return (bVal == btn);
#else
  return ((btn == BTN_GO && bank_a.readByte(BTN_PRESS) % 2) || (btn == BTN_LEFT && getFunctionButton() == 1) || (btn == BTN_RIGHT && getFunctionButton() == 2));
#endif
}
...
void EVShield::getReading(uint16_t *retx, uint16_t *rety) // returnX, returnY to avoid shadowing local x, y
{
  ...
  if (x < min(x1, min(x2, min(x3, x4))) || x > max(x1, max(x2, max(x3, x4))) || y < min(y1, min(y2, min(y3, y4))) || y > max(y1, max(y2, max(y3, y4))))
  {
    *retx = 0;
    *rety = 0;
    return;
  ...
  if (y2 - y1 == 0 || x4 - x1 == 0 || y3 - y4 == 0 || x3 - x2 == 0)
  {
    *retx = 0;
    *rety = 0;
    return;
...
bool EVShieldUART::readAndPrint(uint8_t loc, uint8_t len)
{
  ...
  {
    Serial.print(readLocationByte(i), DEC);
    Serial.print(" ");
  }
  //Serial.println("");
}
